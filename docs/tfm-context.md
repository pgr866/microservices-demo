...
- ⏳ Tarea 4 — validación local de microservicios: en curso. `productcatalogservice`, `shippingservice` y `paymentservice` ya revisados a fondo (quedan 8). `paymentservice` es el primero de los 6 microservicios sin tests al que ya se le han escrito de verdad (Jest) — quedan 5 (`adservice`, `currencyservice`, `emailservice`, `recommendationservice`; `loadgenerator` descartado).
...
- El `Dockerfile` construye limpio; verificar que la imagen final no arrastra herramientas del stage de build (compiladores, `npm`, `pip`) — `currencyservice` ya lo hace bien de fábrica (runtime final Alpine + runtime de Node, sin npm), el resto se revisa según le toque turno. Llevarlo a nivel de producción cuando aplique (imagen base fijada por digest exacto, usuario no-root) — patrón completo ya aplicado y documentado en `productcatalogservice` (Go/distroless) y `paymentservice` (Node/Alpine, ver nota más abajo).
...
**`productcatalogservice`, `shippingservice` y `paymentservice` — ya revisados a fondo, sirven de plantilla de referencia para los 8 restantes:**
